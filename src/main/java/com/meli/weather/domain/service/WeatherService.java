...
public class WeatherService {
    ...
    public BigDecimal calculateRainIntensity(Planet planetA, Planet planetB, Planet planetC) {
        var pABDistance = planetB.location().xAxis().subtract(planetA.location().xAxis()).pow(2)
                .add(planetB.location().yAxis().subtract(planetA.location().yAxis()).pow(2)).sqrt(MathContext.DECIMAL32);

        var pACDistance = planetC.location().xAxis().subtract(planetA.location().xAxis()).pow(2)
                .add(planetC.location().yAxis().subtract(planetA.location().yAxis()).pow(2)).sqrt(MathContext.DECIMAL32);

        var pBCDistance = planetC.location().xAxis().subtract(planetB.location().xAxis()).pow(2)
                .add(planetC.location().yAxis().subtract(planetB.location().yAxis()).pow(2)).sqrt(MathContext.DECIMAL32);
    ...
    private boolean arePlanetsAligned(BigDecimal errorMargin, Location pALocation, Location pBLocation, Location pCLocation) {
        return verifyAlignment(errorMargin, pALocation, pBLocation, pCLocation);
    }

    private boolean arePlanetsAlignedWithSun(BigDecimal errorMargin, Location pALocation, Location pBLocation, Location pCLocation) {
        return verifyAlignment(errorMargin, Location.sunLocation(), pALocation, pBLocation, pCLocation);
    }

    private boolean verifyAlignment(BigDecimal errorMargin, Location... locations) {
    ...
    private BigDecimal triangleArea(Location locationA, Location locationB, Location locationC) {
        var determinantA = (locationA.xAxis().multiply(locationB.yAxis()).multiply(BigDecimal.ONE))
                .add(locationA.yAxis().multiply(locationC.xAxis()).multiply(BigDecimal.ONE))
                .add(locationB.xAxis().multiply(locationC.yAxis()).multiply(BigDecimal.ONE));

        var determinantB = (locationB.yAxis().multiply(locationC.xAxis()).multiply(BigDecimal.ONE))
                .add(locationA.xAxis().multiply(locationC.yAxis()).multiply(BigDecimal.ONE))
                .add(locationA.yAxis().multiply(locationB.xAxis()).multiply(BigDecimal.ONE));

        return determinantA.subtract(determinantB).abs().divide(BigDecimal.valueOf(2), RoundingMode.FLOOR);
    }

    private boolean compareValuesWithErrorMargin(BigDecimal errorMargin, BigDecimal valueA, BigDecimal valueB) {
